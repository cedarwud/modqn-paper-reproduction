...
| `ASSUME-MODQN-REP-012` | Action eligibility and mask semantics | invisible or out-of-topology beams must be invalid actions | accepted | Phase 01 | yes | env, training, export, UI | action catalog uses stable `satellite-major, beam-minor` ordering; eligibility requires the satellite to be above `0°` line-of-sight elevation; invalid actions receive `Q = -∞` before argmax | Includes candidate ordering semantics and the default horizon-visibility proxy |
...
| `ASSUME-MODQN-REP-015` | Checkpoint selection rule | paper does not define best-vs-final checkpoint reporting | accepted | Phase 01 | yes | training, plots, export | primary report uses the final-episode policy; secondary disclosure reports the best evaluation weighted reward checkpoint | Final-episode checkpoint persistence and metadata disclosure are implemented; best-eval checkpoint remains explicitly not-yet-implemented until a dedicated eval loop lands |
