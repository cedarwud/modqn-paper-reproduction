...
| `03` | Bounded paired pilot plus Phase `03B` objective-geometry follow-up complete; Phase `03C-B` static power-MDP audit passed to bounded paired pilot; Phase `03C-C` bounded paired pilot complete and blocked; EE-MODQN still not promoted | Do not claim EE-MODQN effectiveness. Current runtime/eval evidence still shows high rescaling risk, one-beam collapse, fixed denominator, single selected power profile, and no throughput-vs-EE ranking separation. Do not continue this same route with more beam-only episodes or Catfish; any reopen requires an explicit design correction for denominator-sensitive action/power coupling and one-beam collapse. |
